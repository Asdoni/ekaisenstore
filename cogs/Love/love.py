...
class Love(commands.Cog):
    ...
    async def love(self, ctx, user: discord.User):
        ...
        # Fetching avatars and convert them to images
        author_avatar = await self.fetch_image(ctx.user.display_avatar.url)
        ...
        # Paste the author, user and love image into the new image
        ...
        embed.set_footer(text=f"Love from {ctx.user.display_name} to {user.display_name}")
