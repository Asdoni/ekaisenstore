...
class Leonardo(commands.Cog):
    ...
    @app_commands.command(
        name='generate',
        description='Generate an image based on given parameters',
    )
    async def generate(self, ctx, *, user_input: str):
        if ctx.user.id != user_id:
            await ctx.response.send_message("Feature available only for Premium Users")
            return

        # await ctx.response.defer()  # Acknowledge the interaction     
