...
async def check_image_status(generation_id: str):
    ...
    if response.status_code == 200:
        ...
        image_urls = [img.get('url') for img in data_response.get('generations_by_pk', {}).get('generated_images', [])
                      if img.get('url')]
        if image_urls:
...
# Endpoint to generate an image based on given parameters
...
async def generate_image(request: Request):
    ...
    print(f"Sending payload to Leonardo: {payload}")
    try:
        response = requests.post(LEONARDO_URL, json=payload, headers=HEADERS)
        if response.status_code != 200:
            print(f"Error from Leonardo: {response.text}")
